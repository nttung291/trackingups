...
module.exports = {
	...
	formatStatus: (_language, _status, _rev = false) => {
		...
		return _status && _status === 1 ? Configs.default.status[_language][0] : Configs.default.status[_language][1];
	},
	formatStatusDomain: (_status) => {
		return _.find(Configs.default.statusDomain, {value: _status});
	},
	...
	isLink: (_string) => /(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,})/.test(_string),
	isBase64Img: (_string) => {
		_string = _string.substr(0, 11);
		return _string === 'data:image/';
	},
